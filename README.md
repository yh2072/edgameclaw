...
<p align="center">
  ...
  <a href="#简体中文"><strong>简体中文</strong></a><br />
...
上方 **「How It Works」** 中的演示视频与英文版相同，可直接在页面内播放。
...
安装与启动步骤与英文版 **Quick Start** 相同：`Python 3.10+`、`Node.js 18+`，克隆仓库后 `pip install -r requirements.txt`，配置 `.env` 中的 `API_KEY`，运行 `bash start.sh`，浏览器打开 `http://localhost:8000`。

默认推荐通过 [**OpenRouter**](https://openrouter.ai) 调用模型（`API_BASE_URL=https://openrouter.ai/api/v1`）。
...
**[ahafrog.com](https://ahafrog.com)** 是基于 EdGameClaw 构建的托管 SaaS 平台。欢迎试用完整托管体验。

---

## 📖 引用

如果你在研究或项目中使用了 EdGameClaw，请引用（BibTeX 与英文版 **Citation** 相同，见上方代码块）。
